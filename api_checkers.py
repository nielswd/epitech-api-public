...
def infos():
    ...
    if error != {}:
        return json.dumps(error), error['error']['code']
    try:
        r = session.post(server_url+"/?format=json", verify=ssl_verify, cookies = {'language':'fr'})
...
def planning():
    ...
    try:
        r = session.post(server_url+"/intra/planning/load?format=json&start=%s&end=%s" % (start, end), verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
            return json.dumps({"error": {"message": "Connection token is invalid or has expired", 'code':403}}), 403
        if debug:
...
def susie(action=""):
    ...
    try:
        if method == "POST":
            r = session.post(server_url+"/planning/%s/%s/subscribe?format=json" % (params['calendar_id'],_id), verify=ssl_verify, cookies = {'language':'fr'})
        elif method == "DELETE":
            r = session.post(server_url+"/planning/%s/%s/unsubscribe?format=json" % (params['calendar_id'],_id), verify=ssl_verify, cookies = {'language':'fr'})
        elif method == "GET":
            r = session.post(server_url+"/planning/%s/%s/?format=json" % (params['calendar_id'], _id), verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
...
def projects():
    ...
    try:
        r = session.post(server_url+"/module/board/?format=json&start=%s&end=%s" % (start, end),verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
            return json.dumps({"error": {"message": "Connection token is invalid or has expired", 'code':403}}), 403
...
def project():
    ...
    try:
        if method == "GET":
            r = session.post(server_url+"/module/%s/%s/%s/%s/project/?format=json" % (params['scolaryear'], params['codemodule'], params['codeinstance'], params['codeacti']), verify=ssl_verify, cookies = {'language':'fr'})
        elif method == "POST":
            r = session.post(server_url+"/module/%s/%s/%s/%s/project/register?format=json" % (params['scolaryear'], params['codemodule'], params['codeinstance'], params['codeacti']), verify=ssl_verify, cookies = {'language':'fr'})
        elif method == "DELETE":
            r = session.post(server_url+"/module/%s/%s/%s/%s/project/destroygroup?format=json" % (params['scolaryear'], params['codemodule'], params['codeinstance'], params['codeacti']), verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
            if "// Epitech JSON webservice" in r.text:
                return clean_json(r.text), 403
            else:
...
def get_file():
    ...
    if error != {}:
        return json.dumps(error), error['error']['code']
    try:
        r = session.post(server_url+"/module/%s/%s/%s/%s/project/file/?format=json" % (params['scolaryear'], params['codemodule'], params['codeinstance'], params['codeacti']), verify=ssl_verify , cookies = {'language':'fr'})
        if r.status_code == 403:
            if "// Epitech JSON webservice" in r.text:
                return clean_json(r.text), 403
            else:
...
def allmodules():
    ...
    if error != {}:
        return json.dumps(error), error['error']['code']
    try:
        r = session.get(server_url+"/course/filter?format=json&preload=1&location=FR&location=%s&course=%s&scolaryear=%s" %(params['location'], params['course'], params['scolaryear']), verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
            return json.dumps({"error": {"message": "Connection token is invalid or has expired", 'code':403}}), 403
        return clean_json(r.text)
    except Exception as e:
...
def modules():
    ...
    if error != {}:
        return json.dumps(error), error['error']['code']
    try:
        r = session.get(server_url+"/user/#!/netsoul", verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
            return json.dumps({"error": {"message": "Connection token is invalid or has expired", 'code':403}}), 403
...
def module():
    ...
    try:
        ...
        r = session.post(url, verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
            if "// Epitech JSON webservice" in r.text:
                return clean_json(r.text), 403
            return json.dumps({"error": {"message": "Connection token is invalid or has expired", 'code':403}}), 403
...
def marks():
    ...
    if error != {}:
        return json.dumps(error), error['error']['code']
    try:
        r = session.post(server_url+"/user/#!/netsoul", verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
            return json.dumps({"error": {"message": "Connection token is invalid or has expired", 'code':403}}), 403
...
def messages():
    ...
    if error != {}:
        return json.dumps(error), error['error']['code']
    try:
        r = session.post(server_url+"/intra/user/notification/message?format=json", verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
            return json.dumps({"error": {"message": "Connection token is invalid or has expired", 'code':403}}), 403
        return clean_json(r.text)
    except Exception as e:
...
def alerts():
    ...
    if error != {}:
        return json.dumps(error), error['error']['code']
    try:
        r = session.post(server_url+"/intra/user/notification/alert?format=json", verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
            return json.dumps({"error": {"message": "Connection token is invalid or has expired", 'code':403}}), 403
        return clean_json(r.text)
    except Exception as e:
...
def token():
    ...
    try:
        ...
        r = session.post(url, data=payload, verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
            return json.dumps({"error": {"message": "Connection token is invalid or has expired", 'code':403}}), 403
        return clean_json(r.text)
    except Exception as e:
...
def user():
    ...
    if error != {}:
        return json.dumps(error), error['error']['code']
    try:
        r = session.post(server_url+"/user/%s?format=json" %params['user'], verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
            return json.dumps({"error": {"message": "Connection token is invalid or has expired", 'code':403}}), 403
        return clean_json(r.text)
    except Exception as e:
...
def trombi():
    ...
    try:
        ...
        r = session.post(server_url+"/user/filter/user?format=json"+filters, verify=ssl_verify, cookies = {'language':'fr'})
        if r.status_code == 403:
            return json.dumps({"error": {"message": "Connection token is invalid or has expired", 'code':403}}), 403
        return clean_json(r.text)
    except Exception as e:
